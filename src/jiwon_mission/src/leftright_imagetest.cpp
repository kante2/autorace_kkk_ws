// KAZE 기반 좌/우 표지판 매칭 오프라인 테스트 (ROS + launch, 토픽 X)
#include <ros/ros.h>
...
// KAZE 특징 추출기 + 매처
cv::Ptr<cv::Feature2D> g_feat;              // KAZE
cv::BFMatcher g_matcher(cv::NORM_L2, false); // KAZE float → L2 distance
...
// 파라미터
double g_ratio_thresh      = 0.80; // KAZE 매칭 ratio test
...
void testOneImage(const std::string& img_path)
{
    ...
    if (desc_roi.empty())
    {
        ROS_WARN("[INFO] No KAZE features in ROI for %s", img_path.c_str());
        return;
    }
    ...
    ROS_INFO(" KAZE scores ⇒ left: %d, right: %d → %s",
...
int main(int argc, char** argv)
{
    ...
    std::string default_test1     = pkg_path + "/data/left30.png";
    std::string default_test2     = pkg_path + "/data/right30.png";
    ...
    ROS_INFO("KAZE offline test finished.");
    return 0;
}