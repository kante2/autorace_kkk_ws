// 갈림길미션 control PD 제어
// 바깥쪽 추선 추종: 차선 중심점을 왼or오 전체 픽셀의 한지점에 고정
...
// 회전 관련 파라미터
static double g_entry_duration_sec = 2.5;   // 첫 진입 회전 시간
static double g_align_duration_sec = 0.5;   // (지금은 사용 안 함, 남겨만 둠)
static double g_turn_steer_cmd     = 1.0;   // 진입 때 조향 크기 (-1~+1)
...
static double g_target_x_ratio_left  = 0.30;  // 왼쪽 차선 목표 위치 (화면 가로 30%)
static double g_target_x_ratio_right = 0.70;  // 오른쪽 차선 목표 위치 (화면 가로 70%)
...
void mission_AB_init(ros::NodeHandle& nh, ros::NodeHandle& pnh)
{
  ...
  pnh.param<double>("align_duration_sec", g_align_duration_sec, 0.5); // 안 써도 그냥 받아둠
  ...
  ROS_INFO("[AB_turn] entry=%.2fs, turn_steer=%.2f, motor=%.1f",
           g_entry_duration_sec, g_turn_steer_cmd, g_forward_motor_cmd);
...
void mission_AB_step(int dir)
{
  ...
  if (!g_turn_running || (dir != g_current_turn_dir))
  {
    ...
    ROS_INFO("[AB_turn] START: dir=%d, entry=%.2fs (no align phase)",
             g_current_turn_dir, g_entry_duration_sec);
  }
