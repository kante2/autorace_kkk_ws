...
//  - Hardcoded right insertion maneuver for AB sign
...
double g_servo_right = 0.7;    // right turn servo position (0.0~1.0)
...
void mission_ab_right_init(ros::NodeHandle& nh, ros::NodeHandle& pnh)
{
  ...
  pnh.param<double>("ab_right_duration_sec", g_duration_sec, 2.0);
  pnh.param<double>("ab_right_motor_cmd", g_motor_cmd, 1200.0);
  pnh.param<double>("ab_right_servo", g_servo_right, 0.7);
  ...
  ROS_INFO("[mission_ab_right] init motor='%s' servo='%s' duration=%.2fs motor=%.1f servo_right=%.2f",
           ros::names::resolve(g_motor_topic).c_str(),
           ros::names::resolve(g_servo_topic).c_str(),
           g_duration_sec, g_motor_cmd, g_servo_right);
}
...
void mission_ab_right_step()
{
  ...
  std_msgs::Float64 servo_msg; servo_msg.data = g_servo_right;
  ...
  if (dt >= g_duration_sec) {
    ...
    g_done = true;
    ROS_INFO("[mission_ab_right] completed (%.2fs)", dt);
  }
}
