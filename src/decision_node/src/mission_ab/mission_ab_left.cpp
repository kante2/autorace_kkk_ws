...
static double g_target_offset_px  = -80.0;  // 좌측 차선을 향해 안쪽으로 당김 (왼쪽 음수)
...
static double g_run_duration_sec = 8.0;  // 8초 주행
...
static void centerCB(const geometry_msgs::PointStamped::ConstPtr& msg)
{
  ...
  double dx = cx - (g_bev_center_x_px + g_target_offset_px);  // 왼쪽 차선 기준 목표점
