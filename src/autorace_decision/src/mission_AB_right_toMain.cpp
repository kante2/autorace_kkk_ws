...
// 토픽 이름들
static std::string g_topic_right_center_point;  // 오른쪽 차선 중심
...
// 서브스크라이버
static ros::Subscriber g_right_center_sub;
...
static void processDx(double dx)
{
  ...
  ROS_INFO_THROTTLE(0.5,
    "[lane_AB_right][CB+PID] dx=%.1fpx errN=%.3f steer=%.3f v=%.2f (steer_gain=%.3f, Kp=%.4f, Ki=%.4f, Kd=%.4f, color=%d)",
    dx, err_norm, steer_cmd, speed_cmd,
    g_steer_gain, g_pid_kp, g_pid_ki, g_pid_kd, g_lane_color_code);
}

// -------------------- 콜백: 오른쪽 차선 중심 --------------------
static void rightCenterCB(const geometry_msgs::PointStamped::ConstPtr& msg)
{
  // 타임아웃 체크용 시간 저장
...
static void curvatureCenterCB(const std_msgs::Float32::ConstPtr& msg)
{
  ...
  if (g_max_curv <= g_min_curv) {
    ROS_WARN_THROTTLE(1.0,
      "[lane_AB_right] invalid curvature range (min >= max), use base gain");
  ...
  ROS_INFO_THROTTLE(0.5,
    "[lane_AB_right] center_curv=%.4e |k|=%.4e t=%.3f -> steer_gain=%.3f (range=%.3f~%.3f)",
...
{
  ROS_INFO("[lane_AB_right] mission_AB_right_init()");

  // --- 파라미터 로드 ---

  // Topics
  pnh.param<std::string>("topic_right_center_point",
                         g_topic_right_center_point,
                         std::string("/perception/right_center_px"));
  ...
  ROS_INFO("[lane_AB_right] subscribe right_center='%s'",
           ros::names::resolve(g_topic_right_center_point).c_str());
  ROS_INFO("[lane_AB_right] subscribe center_color='%s'",
           ros::names::resolve(g_topic_center_color).c_str());
  ROS_INFO("[lane_AB_right] subscribe is_crosswalk='%s'",
           ros::names::resolve(g_is_crosswalk_topic).c_str());
  ROS_INFO("[lane_AB_right] subscribe curvature_center='%s'",
           ros::names::resolve(g_topic_curvature_center).c_str());
  ROS_INFO("[lane_AB_right] publish motor='%s', servo='%s'",
           ros::names::resolve(g_motor_topic).c_str(),
           ros::names::resolve(g_servo_topic).c_str());

  // BEV 중심 x (오른쪽 차선 기준으로 튜닝)
  ...
  // --- Pub/Sub ---
  g_right_center_sub  = nh.subscribe(g_topic_right_center_point, 20, rightCenterCB);
  ...
  ROS_INFO("[lane_AB_right] mission_AB_right_init done");
}
...
void mission_AB_right_step()
{
  ...
  if (!g_follow_running) {
    ...
    g_follow_start   = now;
    g_sequence_done  = false;
    ROS_INFO("[lane_AB_right] sequence start (%.1fs)", g_follow_duration_sec);
  }
  ...
  if (seq_elapsed >= g_follow_duration_sec) {
    ...
    ROS_INFO_THROTTLE(1.0, "[lane_AB_right] sequence done");
    return;
  }
  ...
  if (g_is_crosswalk && !g_crosswalk_timer_running) {
    ...
    ROS_INFO("[lane_AB_right] is_crosswalk TRUE: start 7s hold");
  }
  ...
  if (g_crosswalk_timer_running) {
    ...
    if (dt < 7.0) {
      ...
      ROS_INFO_THROTTLE(1.0,
        "[lane_AB_right] is_crosswalk: HOLD (t=%.2f/7.0)", dt);
    } else {
      g_crosswalk_timer_running = false;
      ROS_INFO("[lane_AB_right] is_crosswalk: 7s hold finished");
    }
  }
  // -------------------------------------------------------------
  ...
  // 필요하면 디버그 로그 해제
  // ROS_INFO_THROTTLE(0.5,
  //   "[lane_AB_right][loop] have_dx=%d steer_cmd=%.3f servo=%.3f motor=%.1f v=%.2f (steer_gain=%.3f)",
  //   (int)have_dx, steer_cmd, servo_hw, motor_cmd, speed_cmd, g_steer_gain);
}

bool mission_AB_right_is_done()
{
