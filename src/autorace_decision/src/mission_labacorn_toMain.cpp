...
// -------------------- 전역 토픽/퍼블리셔 --------------------
// [MOD-static] main_node 안 다른 미션과 이름 안 부딪히도록 static 처리
static std::string g_topic_scan;
static std::string g_topic_motor;
static std::string g_topic_servo;
static std::string g_topic_marker;
static std::string g_topic_cloud;

static ros::Publisher   g_pub_motor;
static ros::Publisher   g_pub_servo;
static ros::Publisher   g_pub_marker;
...
static double g_front_max_deg = 300.0;
static double g_lane_offset_y = 0.12;   // 한쪽만 있을 때 중심 보정량
...
static double g_steer_sign   = -1.0; // 방향 반전

static double g_motor_min_cmd = 0.0;
static double g_motor_max_cmd = 2000.0;
static double g_motor_gain    = 1500.0; // 1.0 m/s -> 1500 정도면 예전 1500, 1100 근사 가능
...
static bool   g_have_target       = false;   // 현재 프레임에서 타겟이 있는지
static double g_latest_steer_cmd  = 0.0;     // -1 ~ +1
static double g_latest_speed_cmd  = 0.0;     // m/s
...
// main_node.cpp : mission_labacorn_init(nh, pnh) 에서 호출
void mission_labacorn_init(ros::NodeHandle& nh, ros::NodeHandle& pnh)
{
  ROS_INFO("[cluster_follower] mission_labacorn_init()");
  ...
  g_have_target   = false;
  g_latest_steer_cmd = 0.0;
  g_latest_speed_cmd = 0.0;

  ROS_INFO("[cluster_follower] mission_labacorn_init done");
}
