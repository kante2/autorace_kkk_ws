...
private:    
  ...
  int n = points.size();        std::vector<int> labels(n, -1);        
  int cluster_id = 0;
  ...
  for (int i = 0; i < n; i++) {            
          ...
          for (size_t k = 0; k < seed_set.size(); k++) {                
		          int j = seed_set[k];                if (labels[j] != -1) continue;                
		          labels[j] = cluster_id;
    ...
    void scanCallback(const sensor_msgs::LaserScanConstPtr& scan) {        
        ...
        for (auto r : scan->ranges) {            
		        ...
		        // 전방 180도 기준 ±120도 제한            
		        double angle_deg = angle * 180.0 / M_PI;  // 라디안 → 도            
		        // 0~360 범위로 변환            
            ...
            double left_x_sum = 0.0, left_y_sum = 0.0, right_x_sum = 0.0, right_y_sum = 0.0;            
            ...
            if (left_cnt > 0 && right_cnt > 0) {                
                ...
                double weight_left = (double)right_cnt / total_cnt;                
                double weight_right = (double)left_cnt / total_cnt;
            ...
            if (yaw_deg < 0) {                
		            steering = 0.545 + (yaw_deg / 25.0) * 0.545; // 왼쪽을 더 급격하게 회전            
		        } else {                
