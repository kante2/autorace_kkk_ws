...
inline DetectionResult detect(const std::vector<double> &angle_ranges_deg,
  ...
  const std::vector<PointXY> points_xy = polarToCartesian(angle_ranges_deg, dist_ranges);
  const std::vector<int> labels = dbscan(min_pts, eps, points_xy);
  result.centroids = computeCentroids(points_xy, labels);
